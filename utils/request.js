 ...
 * 发送请求
 */
export default function request(api, method, data, {login = true, noVerify = false})
{
  let Url = getApp().globalData.url, header = HEADER;
  if (login) {
    //登录过期自动登录
    if (!util.checkLogin()) return authLogin().then(res => { return request(api, method, data, { login, noVerify}); });
  }
    ...
    wx.request({
      ...
      success: (res) => {
        if (noVerify)
          ...
          util.logout()
          return authLogin().then(res => { return request(api, method, data, { login, noVerify }); });
        } else
          reject(res.data);
      },
      fail: (msg) => {
